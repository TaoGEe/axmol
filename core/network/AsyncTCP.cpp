...
,_sendData(nullptr)
{
    ax::Director::getInstance()->getScheduler()->schedule(AX_SCHEDULE_SELECTOR(AsyncTCP::update), this, 0, false);
}
...
AsyncTCP::~AsyncTCP()
{
    close();
    waitThreadAndClean();
    ax::Director::getInstance()->getScheduler()->unschedule(AX_SCHEDULE_SELECTOR(AsyncTCP::update), this);
}
